...
# There are 5 parts total, split by data collection needs
python run_analysis.py --part 1
```

The script is organized into five parts. Passing `--part N` runs only that part; omitting `--part` runs all five in order. Metrics are written as text logs under `results/metrics/` and charts are exported to `results/plots/`.
...
4. **Artistic Case Studies**: maps a small, deliberately heterogeneous corpus of texts through the byte->codebook->choreography pipeline and visualizes the "choreographic genome" each text produces, measuring how much movement the body traverses per character. Loads only the precomputed plausibility graph, so it runs in seconds. Writes `results/metrics/case_studies.txt` (+ `.json`) and `results/plots/plot_5_choreographic_genome.png`, `plot_6_amplification.png`.
5. **Rendered Choreography Montages**: renders the physicalized choreography for the same corpus: a wide "seven texts, seven bodies" teaser, a time-sampled montage of a contrasting trio (a sonnet, an error log, an Indigenous script), and a chronophotographic long-exposure trail. Uses pyrender, so it takes a few minutes. Writes `results/plots/plot_7_dance_montage.png`, `plot_8_teaser.png`, `plot_9_trail.png`.

The curated case-study corpus and the byte->region mapping used by Parts 4 and 5 live in `run_analysis.py` (`CASE_STUDIES` for Part 4, `CORPUS` for Part 5); edit those to amplify your own texts.

Note that evaluation can be slow. Parts 1-3 collect large motion datasets and can take as long as 4 hours on a consumer-level CPU; Parts 4 and 5 are much lighter, running in seconds and a few minutes respectively.
